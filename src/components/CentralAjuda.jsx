...
function EditorWysiwyg({ value, onChange }) {
  ...
  const toggleHtml = () => {
    if (!modoHtml) {
      // Entrando no modo HTML
      const html = editorRef.current?.innerHTML || '';
      setHtmlBruto(html);
      setModoHtml(true);
    } else {
      // Saindo do modo HTML — aplica no editor
  ...
  const sepStyle = { width: 1, height: 20, background: '#d0d7de', margin: '0 2px', flexShrink: 0 };
  ...
  return (
    <div style={{ border: '1px solid #d0d7de', borderRadius: 8, overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        background: '#f6f8fa', borderBottom: '1px solid #d0d7de', alignItems: 'center',
      }}>
        <button type="button" style={btnStyle(false)} onMouseDown={e => { e.preventDefault(); execCmd('bold'); }} title="Negrito (Ctrl+B)"><b>N</b></button>
        <button type="button" style={btnStyle(false)} onMouseDown={e => { e.preventDefault(); execCmd('italic'); }} title="Itálico (Ctrl+I)"><i>I</i></button>
        <button type="button" style={btnStyle(false)} onMouseDown={e => { e.preventDefault(); execCmd('underline'); }} title="Sublinhado"><u>S</u></button>
        <button type="button" style={btnStyle(false)} onMouseDown={e => { e.preventDefault(); execCmd('strikeThrough'); }} title="Tachado"><s>T</s></button>
        <div style={sepStyle} />
        <button type="button" style={btnStyle(false)} onMouseDown={e => { e.preventDefault(); execCmd('formatBlock', 'h1'); }} title="Título 1">H1</button>
        <button type="button" style={btnStyle(false)} onMouseDown={e => { e.preventDefault(); execCmd('formatBlock', 'h2'); }} title="Título 2">H2</button>
        <button type="button" style={btnStyle(false)} onMouseDown={e => { e.preventDefault(); execCmd('formatBlock', 'h3'); }} title="Título 3">H3</button>
        <button type="button" style={btnStyle(false)} onMouseDown={e => { e.preventDefault(); execCmd('formatBlock', 'p'); }} title="Parágrafo">¶</button>
        <div style={sepStyle} />
        <button type="button" style={btnStyle(false)} onMouseDown={e => { e.preventDefault(); execCmd('insertUnorderedList'); }} title="Lista com marcadores">• Lista</button>
        <button type="button" style={btnStyle(false)} onMouseDown={e => { e.preventDefault(); execCmd('insertOrderedList'); }} title="Lista numerada">1. Lista</button>
        <div style={sepStyle} />
        <button type="button" style={btnStyle(false)} onMouseDown={e => { e.preventDefault(); execCmd('justifyLeft'); }} title="Alinhar à esquerda">⬅</button>
        <button type="button" style={btnStyle(false)} onMouseDown={e => { e.preventDefault(); execCmd('justifyCenter'); }} title="Centralizar">⬛</button>
        <button type="button" style={btnStyle(false)} onMouseDown={e => { e.preventDefault(); execCmd('justifyRight'); }} title="Alinhar à direita">➡</button>
        <div style={sepStyle} />
        <button type="button" style={btnStyle(false)} onMouseDown={e => { e.preventDefault(); inserirLink(); }} title="Inserir link">🔗 Link</button>
        <button type="button" style={btnStyle(false)} onMouseDown={e => { e.preventDefault(); execCmd('unlink'); }} title="Remover link">Unlink</button>
        <div style={sepStyle} />
        <button type="button" style={btnStyle(false)} onMouseDown={e => { e.preventDefault(); inserirImagem(); }} title="Inserir imagem por URL">🖼 Imagem</button>
        <button type="button" style={btnStyle(false)} onMouseDown={e => { e.preventDefault(); inserirVideo(); }} title="Inserir vídeo do YouTube">▶ Vídeo</button>
        <div style={sepStyle} />
        <button type="button" style={btnStyle(false)} onMouseDown={e => { e.preventDefault(); execCmd('insertHTML', '<pre style="background:#f1f3f4;padding:12px;border-radius:6px;font-family:monospace;overflow:auto;">código aqui</pre>'); }} title="Bloco de código">&lt;/&gt;</button>
        <button type="button" style={btnStyle(false)} onMouseDown={e => { e.preventDefault(); execCmd('insertHorizontalRule'); }} title="Linha divisória">──</button>
        <div style={{ flex: 1 }} />
        <button type="button" style={btnStyle(modoHtml)} onClick={toggleHtml} title="Ver/editar HTML fonte">HTML</button>
      </div>
      ...
      {modoHtml ? (
        <textarea
          ...
          style={{
            ...
            fontFamily: 'monospace', fontSize: '0.85em', border: 'none', outline: 'none',
        ...
        <div
          ...
          style={{
            minHeight: 380, padding: '14px 16px', outline: 'none',
            fontSize: '0.95em', lineHeight: 1.7, color: '#1f2328',
            background: '#fff',
          }}
